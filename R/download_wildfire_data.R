# Purpose: webscrape wildfire burned area boundaries from public ARC GIS
...
download_dataset <- function() {
  ...
  url <-
    paste0("https://hub.arcgis.com/api/v3/datasets",
    "/025fb2ea05f14890b2b11573341b5b18_0/downloads/",
  ...
  response <- get_with_retry(url, query = params)
  ...
  if (!inherits(response, "error") && status_code(response) == 200) {
    ...
    previous_files <-
      list.files(save_dir,
                 pattern = "california_recent_burned_areas_.*\\.geojson",
                 full.names = TRUE)
    ...
    filename <-
      paste0("california_recent_burned_areas_", timestamp, ".geojson")
